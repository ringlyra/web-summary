...
Credentialを作り、それらとurlsをiceServersで設定す
...
(Audio / Video) をトラックとして持

 ルームという概念はなく、トラックそれぞれに一意のIDが付いて

いて、それをグローバルなどこからでも取得することができ

 取得していなければ課金もされないので、メディアストリームを

動的につなぎ替えることができるスイッチャーのような役目を
...
ノイズキャンセルなどの連携が用意されているよ

 ベース技術としてはServerless SFU / TURN Serverなのは変

化がなく、ユーザー認証やルーム機能などが含まれていて、即使
...
=:::=::==>:"stun:stun.cloudflare.com:3478""max-bundle""sendonly"getUserMediagetTracksmapaddTransceivertruetrue1
